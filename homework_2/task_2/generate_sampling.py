...
def sampling_decode(model, tokenizer, prompt: str, eos_token_id: int, max_new_tokens: int) -> str:
    ...
    run.track(num_generated, name='num_generated_tokens', context={'prompt': prompt[:30]})
    run.track(elapsed, name='generation_time', context={'prompt': prompt[:30]})
...
if __name__ == '__main__':
    ...
    run = Run(experiment='sampling_generation')
    ...
    for i in range(3):
        story = sampling_decode(
            model, tokenizer,
            prompt=input_text_hedgehog,
            eos_token_id=EOS_TOKEN_ID,
            max_new_tokens=MAX_NEW_TOKENS
        )
        print(f"\n=== Generated hedgehog story {i+1} ===\n")
        print(story)

    for i in range(3):
        json_output = sampling_decode(
            model, tokenizer,
